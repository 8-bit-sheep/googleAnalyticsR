...
You can see what dimensions and metrics are available via the API, you can use this command:
...
If offline, from `v0.2.0` this data is also available offline by typing `meta`.  It does not include the multi-channel metrics as they are not available in the API. 

In API calls, you can call metrics and dimensions with or without the `ga:` prefix (`mcf:` for multi-channel metrics/dimensions.)
...
Below a new client Id is set for both offline and Shiny use.  Set the options before any call to `ga_auth()` or other data fetching calls.
...
In this example, the scopes are also set so a user can authenticate with searchConsoleR at the same time, which is possible as they both use [googleAuthR](http://code.markedmondson.me/googleAuthR/) as a backend.
