---
title: "Reporting API v3"
---

## v3 features

New API features will no longer occur for the v3 API, but it still carries a lot of useful capabilties, such as the multi-channel API.  

It is also currently the only API that has the WALK falg implemented, to avoid sampling via breaking up big data fetches into multiple batched calls.


## Google Anaytics V3 API examples

Examples for using the v3 of the API are shown below.
...
Note you do not need to encode symbols, but may have to encode URLs if you are filtering for those in say ga:pagePath
