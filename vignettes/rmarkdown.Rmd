...
Use `ga_auth_setup()` to create a client.id and then a auth service key.
...
# we don't need to set the client.id as it now takes it from the service key
...
However, the recommend method is to use service accounts and `ga_auth(json_file="auth_key.json")` when using a server since this is easier and more secure in the long run.  Upload the JSON file instead of the cache folder.
