...
[googleAnalyticsR guide online](https://code.markedmondson.me/googleAnalyticsR)
...
* Automatic batching, sampling avoidance with daily walk, multi-account fetching, multi-channel funnel
* Optimisations to make fetching large amounts of data as quickly as possible (batching, caching etc.)
* Management API features
* The `.gamr` file format for sharing modelling analysis of Google Analytics data.
...
This is intended for users who may be familiar with Google Analytics but not with SQL necessary to extract equivalent data from a BigQuery export of Google Analytics 360 data.
...
To use, you need to also install the [`bigQueryR`](https://code.markedmondson.me/bigQueryR/) library.
