...
`ga_model` objects can be saved to the new `.gamr` format.  Loading these files using `ga_model()` and specifying your own Google Analytics viewId, all the data processing, modelling and visualisation steps can be encapsulted to give you the output.  
...
Included within the package are some simple models to demonstrate its use.  
...
The model can also be used to create a Shiny app, as it creates a [Shiny module](https://shiny.rstudio.com/articles/modules.html) of the model.
