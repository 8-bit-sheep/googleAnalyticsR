...
User management follows the recommended workflows outlined in this [Google reference article](https://developers.google.com/analytics/devguides/config/mgmt/v3/user-management). It follows the batching endpoint, to take advantage of the increased quota limits and performance you get if you send in multiple calls at once. For every 30 API calls that are batched, you are only increment 1 API call against your management API quota.  You can have 300 API calls in one batch, but the `googleAnalyticsR` will split it into multiple batches if you have more than 300, so send in the entire vector if you have more than 300. 
