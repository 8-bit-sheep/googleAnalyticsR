...
User management follows the recommended workflows outlined in this [Google reference article](https://developers.google.com/analytics/devguides/config/mgmt/v3/user-management). It follows the batching endpoint, to take advantage of the increased quota limits and performance you get if you send in multiple calls at once. For every 30 API calls that are batched, you only increment one API call against your management API quota.  You can have 300 API calls in one batch, but the `googleAnalyticsR` will split it into multiple batches if you have more than 300, so send in the entire vector even if you have more than 300. 

You first would want to list the users you have attached already to Google Analytics.  Users can be attached at various levels in the hierarchy so you can call the function at various viewing angles.
...
To delete a user from all accounts, web-properties and views takes a few API calls to find all the possible links, which is done behind the scenes of `ga_users_delete()`.  You can pass it multiple emails to delete many at once via batching:
