...
> Note these are different from account filters, which you can manipulate using the management API using the `ga_filter` family of functions.
...
Metric expressions are custom calculated metrics can be calculated on the fly when you supply a named vector with the name of your created custom metric, and the calculation expression you are performing.  You need to supply the `ga:` prefix when creating custom metrics. 

Note these a distinct from the calculated metrics you can create in the web UI. 

```r
my_custom_metric <- c(visitPerVisitor = "ga:visits/ga:visitors")
```

You can then use then within the `metrics` argument, but you also need to supply a `metricFormat` (see [docs](https://developers.google.com/analytics/devguides/reporting/core/v4/rest/v4/reports/batchGet#MetricType)) vector the same length which specifies the type of the metric you have calculated.  These can be:
