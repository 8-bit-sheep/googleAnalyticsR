...
To get started refer to th tutorials linked on the homepage, consult `?google_analytics_4` when within R to see the documentation, and refer to these example queries:
...
```r
google_analytics_4(868768, date_range = c("2016-12-31", "2017-02-01"), metrics = "sessions")
```
...
google_analytics_4(868768, date_range = c(3daysAgo, yesterday), metrics = "sessions")
```

Or use the v4 API shortcuts directly such as `"yesterday"`, `"today"` or `"XDaysAgo"`:

```r
google_analytics_4(868768, date_range = c("5daysAgo", "yesterday"), metrics = "sessions")
```

### Compare date ranges

New to v4 you can compare date ranges and return the two data sets in one call.  Simply send in a 4-length vector of dates in the form `(range1_start, range1_end, range2_start, range3_end)`

```r
google_analytics_4(868768, 
