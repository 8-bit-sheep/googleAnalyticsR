...
test_that("Get Goal list for view", {
  ...
  goals <- ga_goal_list(accountId, webPropId, profileId)
...
test_that("Get Specific Goal", {
  ...
  goal <- ga_goal(accountId, webPropId, profileId , goalId = 1)
...
test_that("Add goal to the view", {
  ...
  response <- ga_goal_add(Goal, 
                          accountId = accountId2, 
                          webPropertyId = webPropId2, 
                          viewId = ga_id2)
  
  expect_equal(response$kind, "analytics#goal")
  ...
  response3 <- ga_goal_update(Goal2, accountId2, webPropId2, ga_id2, 10, method = "PATCH") 
