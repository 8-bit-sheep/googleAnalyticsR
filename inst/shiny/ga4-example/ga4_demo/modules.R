...
#'
#' @return Something
multi_select <- function(input, output, session, type = c("METRIC","DIMENSION")){
  ...
  observe({
    ...
    choice <- meta[meta$type == type & meta$status == "PUBLIC", "name"]
    names(choice) <- meta[meta$type == type & meta$status == "PUBLIC", "uiName"]
